<script lang="ts">
	...
	let geoImages = $derived(
		data.records.filter((img: any) => hasLocation(img.record as Record<string, unknown>))
	);
</script>
...
<div class="relative h-dvh w-full overflow-hidden">
	...
	>
		...
		{#each geoImages as img (img.uri)}
			{@const loc = getLocationFromRecord(img.record as Record<string, unknown>)}
			{#if loc}
				{@const lat = Number(loc.latitude)}
				{@const lng = Number(loc.longitude)}
				{@const thumbUrl = getImageFromRecord(img.record, img.did)}
				{@const label = getLocationLabel(img.record as Record<string, unknown>)}
